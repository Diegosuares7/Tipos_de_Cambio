...
export class EcbPesoMexicanoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getExchangeRate(ExchangeRate.PESO_MEXICANO);
      ...
      return handleProcessError(error.message, 'EcbPesoMexicanoHandlerService');
    }
  }
}
