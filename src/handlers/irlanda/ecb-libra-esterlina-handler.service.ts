...
export class EcbLibraEsterlinaHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getExchangeRate(ExchangeRate.LIBRA_ESTERLINA);
      ...
      return handleProcessError(error.message, 'EcbLibraEsterlinaHandlerService');
    }
  }
}
