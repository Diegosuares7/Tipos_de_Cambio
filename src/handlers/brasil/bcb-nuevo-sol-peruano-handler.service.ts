...
import { BRASIL_BCB, BRASIL_BCB_NUEVO_SOL_PERUANO } from '../../utiles/const-coins';
...
export class BcbNuevoSolPeruanoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScraping(BRASIL_BCB_URL, BRASIL_BCB, BRASIL_BCB_NUEVO_SOL_PERUANO);
      return {
        success: true,
        currency: {
