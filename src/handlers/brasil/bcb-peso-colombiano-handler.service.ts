...
import { BRASIL_BCB, BRASIL_BCB_PESO_COLOMBIANO } from '../../utiles/const-coins';
...
export class BcbPesoColombianoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScraping(BRASIL_BCB_URL, BRASIL_BCB, BRASIL_BCB_PESO_COLOMBIANO);
      return {
        success: true,
        currency: {
