...
import { BRASIL_BCB, BRASIL_BCB_DOLAR_CANADIENSE } from '../../utiles/const-coins';
import { BRASIL_BCB_URL } from '../../utiles/const-url';
import { getWebScraping } from './get-web-scraping';

export class BcbDolarCanadienseHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScraping(BRASIL_BCB_URL, BRASIL_BCB, BRASIL_BCB_DOLAR_CANADIENSE);
      return {
        success: true,
        currency: {
      ...
      return handleProcessError(error.message, 'BcbDolarCanadienseHandlerService');
    }
  }
}
