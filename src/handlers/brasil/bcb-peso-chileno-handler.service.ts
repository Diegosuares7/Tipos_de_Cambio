...
import { BRASIL_BCB, BRASIL_BCB_PESO_CHILENO } from '../../utiles/const-coins';
import { BRASIL_BCB_URL } from '../../utiles/const-url';
import { getWebScraping } from './get-web-scraping';

export class BcbPesoChilenoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScraping(BRASIL_BCB_URL, BRASIL_BCB, BRASIL_BCB_PESO_CHILENO);
      return {
        success: true,
        currency: {
      ...
      return handleProcessError(error.message, 'BcbPesoChilenoHandlerService');
    }
  }
}
