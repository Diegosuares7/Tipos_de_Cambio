...
import { PERU_SBS_PESO_CHILENO } from '../../utiles/const-coins';
import { PERU_SBS_URL_CONTABLE } from '../../utiles/const-url';
import { getWebScrapingContable } from './get-web-scraping';

export class SbsPesoChilenoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScrapingContable(PERU_SBS_URL_CONTABLE, PERU_SBS_PESO_CHILENO);
      ...
      return handleProcessError(error.message, 'SbsPesoChilenoHandlerService');
    }
  }
}
