...
import { PERU_SBS_GUARANI_PARAGUAYO } from '../../utiles/const-coins';
import { PERU_SBS_URL_CONTABLE } from '../../utiles/const-url';
import { getWebScrapingContable } from './get-web-scraping';

export class SbsGuaraniParaguayoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScrapingContable(PERU_SBS_URL_CONTABLE, PERU_SBS_GUARANI_PARAGUAYO);
      ...
      return handleProcessError(error.message, 'SbsGuaraniParaguayoHandlerService');
    }
  }
}
