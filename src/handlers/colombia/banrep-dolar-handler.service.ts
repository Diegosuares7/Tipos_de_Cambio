...
import { COLOMBIA_BANREP_DOLAR } from '../../utiles/const-coins';
import { COLOMBIA_BANREP_URL } from '../../utiles/const-url';
import { COLOMBIA_BANREP_URL_COOKIE } from '../../utiles/const-url';
import { getWebScrapingDolar } from './get-web-scraping';

export class BanrepDolarHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScrapingDolar(
        COLOMBIA_BANREP_URL,
        COLOMBIA_BANREP_URL_COOKIE,
        COLOMBIA_BANREP_DOLAR,
      );
      ...
      return handleProcessError(error.message, 'BanrepDolarHandlerService');
    }
  }
}
