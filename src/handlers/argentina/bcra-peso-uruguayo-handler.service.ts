...
import { ARGENTINA_BCRA_PESO_URUGUAYO } from '../../utiles/const-coins';
import { ARGENTINA_BCRA_URL } from '../../utiles/const-url';
import { getWebScraping } from './get-web-scraping';

export class BcraPesoUruguayoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScraping(ARGENTINA_BCRA_URL, ARGENTINA_BCRA_PESO_URUGUAYO);
      ...
      return handleProcessError(error.message, 'BcraPesoUruguayoHandlerService');
    }
  }
}
