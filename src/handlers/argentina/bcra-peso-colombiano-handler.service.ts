...
import { ARGENTINA_BCRA_PESO_COLOMBIANO } from '../../utiles/const-coins';
import { ARGENTINA_BCRA_URL } from '../../utiles/const-url';
import { getWebScraping } from './get-web-scraping';

export class BcraPesoColombiaHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getWebScraping(ARGENTINA_BCRA_URL, ARGENTINA_BCRA_PESO_COLOMBIANO);
      ...
      return handleProcessError(error.message, 'BcraPesoColombiaHandlerService');
    }
  }
}
