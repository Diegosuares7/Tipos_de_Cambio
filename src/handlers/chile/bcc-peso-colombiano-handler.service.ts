...
import { CHILE_BCC_PESO_COLOMBIANO } from '../../utiles/const-coins';
...
export class BccPesoColombianoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getViaWebScraping(CHILE_BCC_URL_SECONDARY, CHILE_BCC_PESO_COLOMBIANO);
      ...
      return handleProcessError(error.message, 'BccPesoColombianoHandlerService');
    }
  }
}
