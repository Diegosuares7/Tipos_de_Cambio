...
import { CHILE_BCC_PESO_MEXICANO } from '../../utiles/const-coins';
...
export class BccPesoMexicanoHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getViaWebScraping(CHILE_BCC_URL_SECONDARY, CHILE_BCC_PESO_MEXICANO);
      ...
      return handleProcessError(error.message, 'BccPesoMexicanoHandlerService');
    }
  }
}
