...
import { CHILE_BCC_DOLAR_CANADIENSE } from '../../utiles/const-coins';
...
export class BccDolarCanadienseHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getViaWebScraping(CHILE_BCC_URL_SECONDARY, CHILE_BCC_DOLAR_CANADIENSE);
      ...
      return handleProcessError(error.message, 'BccDolarCanadienseHandlerService');
    }
  }
}
