...
import { CHILE_BCC_EURO } from '../../utiles/const-coins';
...
export class BccEuroHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getViaWebScraping(CHILE_BCC_URL_SECONDARY, CHILE_BCC_EURO);
      ...
      return handleProcessError(error.message, 'BccEuroHandlerService');
    }
  }
}
