...
import { CHILE_BCC_REAL } from '../../utiles/const-coins';
...
export class BccRealBrasilenioHandlerService implements CurrencyHandler {
  async getCurrencyData(): Promise<CurrencyProcess> {
    try {
      const { value, date } = await getViaWebScraping(CHILE_BCC_URL_SECONDARY, CHILE_BCC_REAL);
      ...
      return handleProcessError(error.message, 'BccRealBrasilenioHandlerService');
    }
  }
}
